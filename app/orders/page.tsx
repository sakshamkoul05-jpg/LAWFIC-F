    ...
    <>
      <section className="grain bloom relative overflow-hidden border-b border-line">
        <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
          <p className="label text-brass">Your filings</p>
          <h1 className="mt-6 max-w-2xl font-display text-[clamp(30px,4.2vw,44px)] leading-[1.08] text-bone">
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        {orders.length === 0 ? (
          <div className="rounded-lg border border-line bg-ink-2 px-6 py-16 text-center">
            <p className="text-[15px] text-ash">You have not started a filing yet.</p>
            <Link
              href="/services"
              className="mt-7 inline-block rounded-full bg-brass px-6 py-3 text-sm font-medium text-ink transition-colors hover:bg-brass-hi"
            >
              Browse services
            </Link>
          </div>
            ...
            {orders.map((o) => {
              ...
              return (
                <Link
                  key={o.id}
                  href={`/orders/${o.id}`}
                  className="card group grid gap-4 border-0! p-6 md:grid-cols-[1.5fr_1fr_auto] md:items-center md:gap-8"
                >
                  <div className="min-w-0">
                    <p className="font-mono text-[11.5px] tracking-[0.08em] text-slate">
                      {o.reference}
                    </p>
                    <h2 className="mt-2 font-display text-[20px] leading-snug text-bone">
                  ...
                  <div>
                    <p className="label text-slate">Amount</p>
                    <p className="mt-1 font-mono text-[14px] text-bone tnum">
  ...
  return (
    <section className="grain bloom relative min-h-[70vh] overflow-hidden">
      <div className="relative z-2 mx-auto max-w-6xl px-5 py-24 sm:px-8">
        <p className="label text-brass">Your filings</p>
        <h1 className="mt-6 max-w-xl font-display text-[clamp(28px,4vw,40px)] leading-[1.1] text-bone">
          {title}
        </h1>
