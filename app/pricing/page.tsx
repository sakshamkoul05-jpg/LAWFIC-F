    ...
    <>
      <section className="grain bloom relative overflow-hidden border-b border-line">
        <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 lg:py-24">
          <Reveal>
            <p className="label text-brass">Pricing</p>
            ...
            <Reveal key={plan.id} delay={i * 0.07}>
              <div
                className={`relative flex h-full flex-col overflow-hidden rounded-xl border p-7 ${
                  plan.featured
                    ? "border-brass-lo bg-gradient-to-b from-surface-2 to-ink-2 shadow-2xl shadow-black/50"
                    : "border-line bg-ink-2"
                }`}
              >
                {plan.featured && (
                  <>
                    <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-brass to-transparent" />
                    <span className="label absolute right-5 top-5 rounded-full border border-brass-lo bg-brass/10 px-2.5 py-1 text-brass">
                      Most chosen
                    </span>
                  </>
                )}

                <h2 className="font-display text-[24px] leading-tight text-bone">{plan.name}</h2>
                <p className="mt-2.5 min-h-[42px] text-[14px] leading-relaxed text-ash">
                  {plan.tagline}
                </p>

                <div className="mt-6 border-y border-line py-5">
                  {plan.monthlyPaise === null ? (
                    <p className="font-display text-[34px] leading-none text-brass">₹0</p>
                  ) : (
                    <p className="font-display text-[34px] leading-none text-bone tnum">
                ...
                <p className="mt-5 text-[13px] leading-relaxed text-slate">{plan.bestFor}</p>

                <ul className="mt-6 flex flex-col gap-3">
                  {plan.includes.map((f) => (
                    <li key={f} className="flex gap-3 text-[14px] leading-relaxed text-ash">
                      <svg width="15" height="15" viewBox="0 0 16 16" fill="none" className="mt-1 shrink-0" aria-hidden>
                        <path d="m3.5 8.4 3 3 6-6.6" stroke="var(--color-brass)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                      {f}
                    </li>
                  ))}
                ...
                {plan.excludes && (
                  <ul className="mt-4 flex flex-col gap-2 border-t border-line pt-4">
                    {plan.excludes.map((f) => (
                      <li key={f} className="flex gap-3 text-[12.5px] leading-relaxed text-slate">
                        <span className="mt-2 h-px w-3 shrink-0 bg-line-3" aria-hidden />
                        {f}
                      </li>
                    ))}
                  </ul>
                ...
                <Link
                  ...
                  className={`mt-auto block rounded-full px-6 py-3 text-center text-sm font-medium transition-colors ${
                    plan.featured
                      ...
                      : "border border-line-2 text-bone hover:border-brass-lo"
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        <Reveal>
          <p className="label text-brass">Worked examples</p>
          <h2 className="mt-5 max-w-2xl font-display text-[clamp(26px,3.6vw,36px)] leading-tight text-bone">
            What a filing actually costs, both halves shown
          </h2>
          <p className="mt-5 max-w-2xl text-[15.5px] leading-relaxed text-ash">
              ...
              <thead>
                <tr className="bg-surface/60">
                  {["Service", "Government fee", "LAWFIC fee", "Turnaround"].map((h) => (
                    <th key={h} className="label border-b border-line px-6 py-4 text-slate">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {services.map((s) => (
                  <tr key={s.slug} className="border-b border-line last:border-0">
                    <td className="px-6 py-4">
                      <Link href={`/services/${s.slug}`} className="text-[14.5px] text-bone hover:text-brass">
                        {s.name}
                      </Link>
                    </td>
                    <td className="px-6 py-4 text-[13.5px] leading-snug text-ash">
                      {s.fee.government}
                    </td>
                    <td className="px-6 py-4 font-mono text-[14px] text-brass tnum">
                      {s.fee.professional}
                    </td>
                    <td className="px-6 py-4 text-[13.5px] text-slate">{s.turnaround}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        <Reveal>
          <p className="label text-brass">Our commitments on price</p>
          <h2 className="mt-5 max-w-2xl font-display text-[clamp(26px,3.6vw,36px)] leading-tight text-bone">
        ...
        <div className="mt-12 grid gap-px overflow-hidden rounded-lg border border-line bg-line sm:grid-cols-2">
          {pricingCommitments.map((c, i) => (
            <Reveal key={c.title} delay={i * 0.06}>
              <div className="flex h-full flex-col bg-ink-2 p-7">
                <span className="font-mono text-[12px] tracking-[0.14em] text-brass">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <h3 className="mt-4 font-display text-[20px] leading-snug text-bone">{c.title}</h3>
                <p className="mt-3 text-[14px] leading-relaxed text-ash">{c.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-24 sm:px-8">
        <Reveal>
          <p className="label text-brass">Questions about billing</p>
        </Reveal>

        <div className="mt-10 flex flex-col gap-px overflow-hidden rounded-lg border border-line bg-line">
          {pricingFaq.map((f, i) => (
            <Reveal key={f.q} delay={i * 0.04}>
              <div className="grid gap-4 bg-ink-2 p-7 md:grid-cols-[1fr_1.4fr] md:gap-10">
                <h3 className="font-display text-[18px] leading-snug text-bone">{f.q}</h3>
        ...
        <Reveal>
          <div className="mt-14 rounded-lg border border-line-2 bg-surface/40 px-8 py-10 text-center">
            <h3 className="font-display text-[24px] text-bone">Not sure which fits?</h3>
