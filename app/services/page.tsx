    ...
    <>
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[1.15fr_1fr] lg:py-24">
          ...
          <Reveal delay={0.1}>
            <dl className="mt-12 grid max-w-2xl grid-cols-3 gap-px overflow-hidden border border-border">
              {[
                ...
                <div key={k} className="bg-surface px-5 py-5">
                  <dt className="type-data text-[28px] leading-none text-primary">{v}</dt>
                  <dd className="type-label mt-2">{k}</dd>
                </div>
              ))}
            </dl>
          </Reveal>
        ...
        <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-5 py-3 sm:px-8">
          {categories.map((c) => (
            <a
              key={c.id}
              href={`#${c.id}`}
              className="flex shrink-0 items-center gap-2 rounded-full border border-border px-3.5 py-1.5 text-[12px] text-muted transition-colors hover:border-primary hover:text-foreground"
            >
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        <Reveal>
          <div className="mb-8 flex flex-wrap items-baseline gap-x-4 gap-y-2">
            <h2 className="type-h1 text-foreground">Live now</h2>
        ...
        <div className="grid gap-px overflow-hidden border border-border sm:grid-cols-2">
          {livePages.map((s, i) => (
            <Reveal key={s.slug} delay={i * 0.05}>
              <Link href={`/services/${s.slug}`} className="group flex h-full flex-col bg-surface p-7 transition-colors hover:bg-surface-2">
                <div className="flex items-start justify-between gap-4">
                  <p className="type-label text-primary">{s.category}</p>
                  <span className="type-label">{s.turnaround}</span>
                </div>
                <h3 className="type-h2 mt-5 text-foreground">{s.name}</h3>
                <p className="mt-3 text-[14px] leading-relaxed text-muted">{s.tagline}</p>
                <div className="mt-auto flex items-end justify-between gap-4 pt-8">
                  <div>
                    <p className="type-label">Our fee</p>
                    <p className="type-data mt-1 text-[15px] text-foreground">{s.fee.professional}</p>
                  </div>
                  <span className="text-[13px] text-primary transition-opacity group-hover:opacity-100 opacity-70">
                    View details
                  </span>
                </div>
              </Link>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-24 sm:px-8">
        <Reveal>
          <div className="mb-10 flex flex-wrap items-baseline gap-x-4 gap-y-2 border-t border-border pt-14">
            <h2 className="type-h1 text-foreground">The full catalogue</h2>
            <p className="text-[14px] text-muted">
            ...
            <div key={c.id} id={c.id} className="scroll-mt-32">
              <Reveal>
                <div className="mb-6 flex items-start gap-4">
                  <span className="mt-0.5 grid size-11 shrink-0 place-items-center rounded-lg border border-border bg-surface text-primary">
                    <CategoryIcon name={c.icon} size={22} />
                  </span>
                  <div>
                    <h3 className="type-h2 text-foreground">{c.name}</h3>
              ...
              <div className="grid gap-px overflow-hidden border border-border sm:grid-cols-2 lg:grid-cols-3">
                {c.services.map((s) =>
                  s.status === "live" ? (
                    <Link
                      key={s.slug}
                    ...
                    >
                      <span className="flex items-center gap-2">
                        ...
                        <span className="type-data rounded border border-success/20 bg-success-light px-1.5 py-0.5 text-[9px] text-success">
                          Live
                        </span>
                      </span>
                    ...
                    <div key={s.slug} className="bg-surface-2 p-5 opacity-50">
                      <span className="flex items-center gap-2">
                        <span className="text-[14px] text-muted">{s.name}</span>
                        <span className="type-data rounded border border-border px-1.5 py-0.5 text-[9px] text-subtle">
                          Soon
                        </span>
                      </span>
                      <span className="mt-1.5 block text-[12.5px] leading-snug text-muted">
        ...
        <Reveal>
          <div className="mt-16 border border-border bg-surface px-8 py-12 text-center">
            <h3 className="type-h2 text-foreground">Not on the list?</h3>
