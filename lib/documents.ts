 ...
 *
 * `href` points either at a full service page (when `live`) or at that
 * document's own page under /document/<slug>. It used to point at
 * `/document#<slug>` for everything not live — an anchor to an element that
 * did not exist, so two-thirds of the catalogue were dead links.
 */
